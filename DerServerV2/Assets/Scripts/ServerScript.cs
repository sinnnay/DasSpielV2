...
public class ServerScript : MonoBehaviour {
    ...
    // Die Client Verbindungen, mit denen der Server eine Verbindung hat
    private List<TcpClient> connectedClients = new List<TcpClient>();

    // Die Worker-Threads des Servers für die Client-Verbindungen
    private List<Thread> connectedClientThreads = new List<Thread>();
    ...
    void StartListening()
    {
        ...
        try
        {
            ...
            {                            
                ...
                connectedClientThreads.Add(clientThread);
    ...
    private void WorkWithClient(object client)
    {
        ...
        catch (EndOfStreamException e)
        {
            ...
            if (connectedClient.Connected)
                connectedClient.Close();
            if (connectedClients.Contains(connectedClient))
                connectedClients.Remove(connectedClient);
            if (connectedClientThreads.Contains(Thread.CurrentThread))
                connectedClientThreads.Remove(Thread.CurrentThread);
            Debug.Log(e);
            return;
        }
...
*/
    ...
    {
        Debug.Log("in RemoveAllClientConenctions");
        ...
        Debug.Log("All clientConnections have been removed from the list");
    }

    void RemoveAllWorkerThreads()
    {
        Debug.Log("in RemoveAllWorkerThreads");
        foreach (var workerThread in connectedClientThreads)
        {
            if (workerThread.IsAlive)
                workerThread.Abort();
        }
        connectedClientThreads.Clear();
        Debug.Log("All workerThreads have been removed from the list");
    }
